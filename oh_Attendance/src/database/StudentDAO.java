...
//		}
	...
	public boolean add_stu_info(StudentVo s) {
		try {
			...
			String query = "INSERT INTO STUDENT(stuNumber, stuName, sex, birth,address;guardian1,  guardian1_call, guardian2, guardian2_call, stu_memo) "
					+ "values('" + s.getStuNumber() + "','" + s.getStuName() + "','" + s.getSex() + "','" + s.getBirth()
					+ "','" + s.getAddress() + "','" + s.getEnter_date() + "','" + s.getStudent_call() + "','"
					+ s.getGuardian1() + "','" + s.getGuardian1_call() + "','" + s.getGuardian2() + "','"
					+ s.getGuardian2_call() + "')";
			System.out.println("SQL : " + query);
			rs = stmt.executeQuery(query);
			System.out.println("rs.getRow() : " + rs.getRow());
