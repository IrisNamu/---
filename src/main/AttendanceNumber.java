	...
	public AttendanceNumber() {
		...
		backBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				dispose();
				AttendanceMain attendance = new AttendanceMain(); // 홈화면 호출
		...
		btnOk.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if ((InputNumber.getText().length() > 4) || (InputNumber.getText().length() < 4)) {
					...
					StudentVo vo = new StudentVo(HomeLogin.getUserID(), InputNumber.getText(), dateLab.getText(), attendance_stuatus,
							timeLab.getText());

					boolean b = dao.Insert_attendance_Info(vo);

					StudentDAO dao = new StudentDAO();
					ArrayList<StudentVo> list = dao.number_name(HomeLogin.getUserID(), InputNumber.getText());
